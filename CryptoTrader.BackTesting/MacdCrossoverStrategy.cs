...
{
    internal class MacdCrossoverStrategy : Strategy
    {
        private int _crossAboveId;
        private int _crossBelowId;

        public MacdCrossoverStrategy(BinanceContext context) : base(context)
        {
            var indicator = GetIndicators<MacdCrossoverAnalyzer>().FirstOrDefault();
            if (indicator != null)
            {
